...
{\small Anomly, Inc. \quad\textbullet\quad May 2026}
...
{\small \href{mailto:ry@anomly.com}{ry@anomly.com}}\\[0.5cm]
{\small 2026-05-06}

\vfill

{\small\itshape Reproducible source: \url{https://github.com/anomly-labs/mosyne-bposit/tree/main}}\\[0.3cm]
{\footnotesize Hardware: RTX 5090 (Blackwell sm\_120) and RTX 3090 (Ampere sm\_86).}\\[0.4cm]
{\footnotesize\copyright\ 2026 Ry Bruscoe and Anomly, Inc. \quad Licensed under the Apache License, Version 2.0.}
...
John L.\ Gustafson \cite{gustafson_endoferror} --- offers two
properties relevant here. First, \emph{tapered precision}: posits give
...
\begin{itemize}
    \item A complete bounded-posit ALU running as CUDA
          ...
          a Python rational-arithmetic reference across 33
...
\cite{gustafson_endoferror} caps the regime field at $r_S = 6$ bits,
fixing the dynamic range at $2^{-48}$ to $2^{48}$. \bpsixteen{}'s
...
reaches $312.1$ \TFLOPS{} on the 5090 INT8 IMMA path (vs
$30.2$ \TFLOPS{} on the RTX 3090 at the same shape, a $10\times$
...
The pipeline presented here is one of three implementation fidelities
of the bposit arithmetic stack at Anomly, Inc. The other two are
hardware: an FPGA bitstream targeting the Xilinx Alveo U200 (PCIe
accelerator, programmed and validated in fabric) and a custom posit
ASIC submitted to the SkyWater 130\,nm shuttle through ChipFoundry,
with first silicon (100 prototype dies) expected to return in Q2 2026.
The 256-bit \quire{} layout used in this paper (1 sign + 63 carry +
96 integer + 96 fraction bit fields) is reduced-to-practice in all
three implementations from the same architectural specification.
Energy-efficiency characterisation of the ASIC, and a bit-exact
cross-implementation equivalence proof against the CUDA pipeline,
are reserved for a follow-up paper after silicon arrives.
...
The \quireTSF{}-to-\bpsixteen{} encoder currently truncates at the
$15$-bit boundary rather than rounding to nearest-even; this matches
the truncation in the Python reference and simplifies bit-exact
testing. A spec-correct round-to-nearest-even pass is future work.

Throughput parity is shape-dependent. At small-batch decode of small
models (e.g., \textsc{Qwen2.5-Coder-3B}'s $D = 2048$ FFN), BF16 HMMA
is faster than INT8 IMMA on the RTX 3090 by approximately $22\,\%$;
the win on memory and reproducibility persists.
...
$1\,\%$; a full multi-layer \textsc{Qwen} or \textsc{Llama} forward
pass on the pipeline; a PyTorch custom op exposing the IMMA path for
external use; and a silicon implementation embodying the same
...
\texttt{kernels/run\_public\_tests.sh} runs the bit-
exact suite in under five minutes on an RTX 5090, including the
determinism head-to-head (\Cref{tab:repro}), the matmul shape sweep
(\Cref{tab:matmul}), the real \textsc{Qwen2.5} layer pass
(\Cref{tab:qwen}), and the W8A8 PTQ progression (\Cref{tab:ptq}).
\texttt{deploy/5dst\_cuda/bench\_robust\_sweep.sh} reproduces the
multi-trial mean $\pm$ stddev sweep across all $11$ matmul shapes;
\texttt{scripts/capture\_5090\_benches.sh} pauses a co-resident
\texttt{vLLM} workload, runs both suites on the freed GPU, and
restarts \texttt{vLLM} on exit.
...
\bibitem{rovelli_smolin_1995}
C.~Rovelli and L.~Smolin.
\newblock Discreteness of area and volume in quantum gravity.
\newblock \emph{Nuclear Physics B}, 442(3):593--619, 1995.

\bibitem{ambjorn_loll_1998}
J.~Ambjørn and R.~Loll.
\newblock Non-perturbative Lorentzian quantum gravity, causality and
topology change.
\newblock \emph{Nuclear Physics B}, 536(1--2):407--434, 1998.
