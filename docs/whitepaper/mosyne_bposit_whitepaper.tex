...
layers $\times$ \{gate, up, down\}-proj). Extending the swap to
attention $Q$/$K$/$V$/$O$ projections regresses end-to-end throughput
by $-13$ to $-16\,\%$ because the K/V shapes ($1536\to 256$ for
\textsc{Qwen}'s grouped-query attention) land in the same skinny-N
regime where \texttt{cuBLASLt}'s INT8 IMMA heuristic underperforms BF16
HMMA on consumer Blackwell (\Cref{tab:matmul}); attention quantisation
is therefore not part of this paper's deployment recommendation and
remains gated on the \texttt{CUTLASS} small-$M$ template work in
\Cref{sec:limits}. Reproducible via
...
of \Cref{tab:matmul}; and a silicon implementation embodying the same
