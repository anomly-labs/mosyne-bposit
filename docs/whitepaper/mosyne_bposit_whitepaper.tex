...
\author{Ry Bruscoe\\Anomly, Inc.\\\href{mailto:ry@anomly.com}{ry@anomly.com}}
\date{2026-07-15}
...
{\small Anomly, Inc. \quad\textbullet\quad July 2026}
...
{\footnotesize Hardware: RTX 5090 (Blackwell sm\_120) and RTX 3090 (Ampere sm\_86).}\\[0.5cm]
{\footnotesize\textcopyright{} 2026 Anomly, Inc. All rights reserved.}
...
per-matrix $2\times$ weight-memory reduction (8-bit posit weights vs
16-bit BF16) and the bit-exact reproducibility property hold at every
shape regardless of throughput; whole-model memory reduction under
the default FFN-only replacement policy is $\sim 30{-}35\%$, and
approaches $2\times$ when attention is also quantised.
...
\bpsixteen{} is the 16-bit variant we focus on. The leading bit is
the sign $s$; the remaining 15 bits encode a magnitude in posit's
two's-complement-style sign representation. When $s = 1$ those 15
bits are first replaced by their 15-bit two's complement; in both
cases the regime $k$, exponent $e$, and fraction $f$ are then
decoded from the (possibly complemented) 15 bits, and the magnitude
is
\[
    |\mathrm{value}| \;=\; \mathrm{useed}^{\,k} \cdot 2^{\,e} \cdot (1 + f).
\]
The signed value is $+|\mathrm{value}|$ when $s = 0$ and
$-|\mathrm{value}|$ when $s = 1$. With three exponent bits the
per-regime base is $\mathrm{useed} = 2^{2^3} = 256$. The
\emph{bounded} variant \cite{gustafson_endoferror}, aligned with the
Posit Working Group draft standard \cite{posit_2017}, caps the
regime field at $r_S = 6$ bits, fixing the dynamic range at
$2^{-48}$ to $2^{48}$. \bpsixteen{}'s precision is comparable to
FP16 and BF16: roughly $3.5$ decimal digits.

\paragraph{A common shorthand that is wrong.} The single-formula
expression
\[
    \mathrm{value} \;\stackrel{?}{=}\; (-1)^{s} \cdot \mathrm{useed}^{\,k}
    \cdot 2^{\,e} \cdot (1 + f)
\]
appears in several recent posit papers but gives the wrong magnitude
for negative posits, because it skips the two's-complement step on
the trailing bits.\footnote{John L.\ Gustafson, personal
communication, 2026-05; see also the reference implementation in our
public source. The shorthand is structurally identical to the
IEEE-754 sign-magnitude formula, which is likely why it propagates:
it ``looks right'' to a reader familiar with float and the
arithmetic happens to work out for positive values.} To see the
discrepancy concretely, consider the 6-bit standard posit
\texttt{1\,01\,10\,1} ($\mathrm{eS}{=}2$, $\mathrm{useed}{=}16$).
Read off the bits without complementing: $s{=}1$, $k{=}{-}1$,
$e{=}2$, $f{=}1/2$. The shorthand evaluates to
\[
    (-1)^{1} \cdot 16^{-1} \cdot 2^{2} \cdot (1 + \tfrac{1}{2})
    \;=\; -\,\tfrac{3}{8}.
\]
The correct two-case decode replaces the trailing \texttt{01101}
with its 5-bit two's complement \texttt{10011}, which decodes to
$k{=}0$, $e{=}1$, $f{=}1/2$, giving magnitude
$16^{0} \cdot 2^{1} \cdot (1 + \tfrac{1}{2}) = 3$ and signed value
$-3$. The two formulations differ by a factor of $8$ — and the
discrepancy grows with the regime, so any negative posit whose magnitude
is far from $1$ is decoded with a wildly wrong value under the
shorthand. We adopt the two-case decode throughout this paper and in
our reference implementation, and call this out explicitly in the
hope of arresting the misformulation's spread.
...
% Whitepaper rev (2026-05-18): two related fixes for this figure.
% (1) the axis was semilogyaxis, but xbar with a log x-scale
% needs semilogxaxis — the prior combo crashed pgfplots in recent
% TeXLive versions and rendered the axis without any bars.
% (2) bar labels use explicit \node placement instead of the
% point-meta=explicit-symbolic + nodes-near-coords pattern, which
% interacts badly with \% inside bracketed symbolic text.
\addplot[draw=black!70, fill=barlose, fill opacity=0.7]
    coordinates {(0.50,pertensor)};
\node[font=\tiny, black!75, anchor=west, xshift=2pt]
    at (axis cs:0.50,pertensor) {50\% (clipping blow-up)};
\addplot[draw=black!70, fill=barpar, fill opacity=0.7]
    coordinates {(0.247,perchannel)};
\node[font=\tiny, black!75, anchor=west, xshift=2pt]
    at (axis cs:0.247,perchannel) {24.7\%};
\addplot[draw=black!70, fill=barwin, fill opacity=0.7]
    coordinates {(0.0345,pertoken)};
\node[font=\tiny, black!75, anchor=west, xshift=2pt]
    at (axis cs:0.0345,pertoken) {3.45\%};
\addplot[draw=black!70, fill=barfill!40, fill opacity=0.6,
         postaction={pattern=north east lines, pattern color=black!50}]
    coordinates {(0.01,gptq)};
\node[font=\tiny, black!75, anchor=west, xshift=2pt]
    at (axis cs:0.01,gptq) {$\le 1\%$ (target)};
\end{semilogxaxis}
\end{tikzpicture}
