...
\bpsixteen{} is the 16-bit variant we focus on. A posit bit pattern is,
first and foremost, a \emph{signed two's-complement integer}, and its value is
obtained by interpreting that integer: if the sign bit is set, first take the
two's complement of the whole word, then read the regime $k$, exponent $e$,
and fraction $f$ off the resulting \emph{magnitude} pattern, giving
\[
    \mathrm{value} = \mathrm{sign} \cdot \mathrm{useed}^k \cdot 2^e \cdot (1 + f),
    \qquad \mathrm{sign} \in \{+1, -1\},
\]
per the Posit\textsuperscript{TM} Standard (2022) general formula. (A common
shorthand reads $k,e,f$ directly off the raw bits and prepends $(-1)^{\mathrm{sign}}$,
treating the posit as sign-magnitude; that form is \emph{wrong for negative
values} because it skips the two's-complement step, and the error grows with
the regime.) The regime, exponent, and fraction together occupy the 15
